...
✍ Los Argonianos tienen una fuerte inmunidad al veneno, su regeneración es más avanzada y algunos pueden camuflajearse hasta tal punto de lograr la invisibilidad.""",
    ...
    "Necromano": """Malditos por la desgracia y la calamidad, los Necromanos son los magos oscuros de la muerte. En tiempos remotos, un grupo de magos se dedicó a descifrar los misterios de los seres divinos y, tras una traición, consiguieron una maldición que condenó a la raza humana. Sin embargo, al ser magos, su magia pudo sostener la maldición y convertirla en su nuevo arma, dominando a los muertos; así, los Nigromantes se convirtieron en una nueva raza.
...
def botones_confirmar_nombre():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔙 Volver a elegir raza", callback_data="volver_raza")]
    ])
...
async def seleccionar_genero(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Limpiar datos anteriores si se reinicia el proceso
    jugador["genero"] = None
    jugador["raza"] = None
    jugador["nombre"] = None
...
async def genero_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Protección: confirmar sin género seleccionado
    if data == "confirmar_genero" and not jugador["genero"]:
        await query.answer("Primero selecciona un género.", show_alert=True)
        return

    if data.startswith("genero_"):
        jugador["genero"] = "Masculino" if data == "genero_M" else "Femenino"
        await query.edit_message_caption(
            caption=f"Has seleccionado el género: {jugador['genero']}",
        ...
        # Usamos edit_message_caption en lugar de edit_message_media
        await query.edit_message_caption(
...
async def raza_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("raza_"):
        ...
        # Premium -> solo descripción, sin confirmación
        ...
        # Ángeles y Demonio -> puramente informativos
    ...
    # =========================
        ...
        if not jugador["raza"]:
            await query.answer("Primero selecciona una raza.", show_alert=True)
            return
        await query.edit_message_caption(
...
async def set_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Colapsar espacios antes de normalizar
    nombre_temp = ' '.join(nombre_temp.split())
...
async def confirmar_nombre_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "confirmar_nombre":
        if not jugador["nombre_temp_norm"]:
            await query.answer("Primero escribe un nombre con /name", show_alert=True)
            return
...
print("BOT ENCENDIDO")
app.run_polling()    "Argoniano": """Una raza mítica, parientes de los Draconianos y adoradores del Dios del pantano. Poseen una variación similar a la humana con aspectos de reptil; son conocidos como shamanes y asesinos de las sombras. Los Argonianos han sobrevivido a los peligros del continente durante siglos. Aunque son ajenos a las costumbres de otras razas, contribuyeron a la paz del continente en la Gran Guerra con su magia ancestral.
